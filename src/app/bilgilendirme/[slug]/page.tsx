...
export default async function BilgilendirmeDetayPage({ params }: Props) {
  ...
  return (
    <PageShell usern={session.usern}>
      <main style={{ maxWidth: 900, margin: '0 auto', padding: '28px 20px' }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: 'var(--text)', marginBottom: 24 }}>{row.bilgilendirme_gorsel_baslik}</h1>
        <div style={{ backgroundColor:'var(--surface)',border:'1px solid var(--border)',borderRadius:16,padding:'28px 32px',lineHeight:1.85,color:'var(--text)',fontSize:14 }}>
          <p style={{ fontWeight:700,marginBottom:12,color:'var(--text)' }}>{row.bilgilendirme_baslik}</p>
          <div dangerouslySetInnerHTML={{ __html: row.bilgilendirme_icerik }} />
        </div>
      </main>
    </PageShell>
  );
