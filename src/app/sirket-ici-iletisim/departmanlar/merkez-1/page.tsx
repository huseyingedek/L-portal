...
export default async function Merkez1Page() {
  ...
  let dbError = false;
  try {
    const [r] = await db.query('SELECT * FROM DEPARTMANLAR_DAHILI_HATLAR WHERE merkez = 1') as [Dahili[], unknown];
    rows = r;
  } catch { dbError = true; }
  ...
  return (
    <PageShell usern={session.usern}>
      <main style={{ maxWidth: 1100, margin: '0 auto', padding: '28px 20px' }}>
        <h1 style={{ fontSize: 20, fontWeight: 700, color: 'var(--text)', marginBottom: 24 }}>Merkez 1 — Çalışanlar Listesi</h1>
