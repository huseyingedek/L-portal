...
export default function KampanyaEklePage() {
    ...
    script.onload = () => {
      ...
      if (containerRef.current && win.ClassicEditor) {
        win.ClassicEditor.create(containerRef.current, {
          ...
          ckfinder: { uploadUrl: '/api/upload?module=calisankampanyalari' },
        }).then((editor: unknown) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/kampanyalar/ekle', {
      ...
      body: JSON.stringify({ kampanya_gorsel_baslik: gorselBaslik, kampanya_baslik: baslik, kampanya_icerik: icerik }),
    });
  ...
  return (
    <PageShell>
      <div className="df-page-narrow">
        <h1 style={{ fontSize: 20, fontWeight: 700, color: 'var(--text)', marginBottom: 24 }}>Kampanya Ekle</h1>
