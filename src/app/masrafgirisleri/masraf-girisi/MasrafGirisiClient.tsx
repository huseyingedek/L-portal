...
const CURRENCIES = [
  { value: '0', label: 'TL' }, { value: '1', label: 'USD' },
  { value: '2', label: 'EUR' }, { value: '3', label: 'AU' },
  { value: '4', label: 'GBP' }, { value: '5', label: 'CHF' },
];
...
export default function MasrafGirisiClient() {
  const searchParams = useSearchParams();
  const docnumb = searchParams.get('docnumb') || '';
  const showSharedRows = docnumb !== '';
  const today = new Date().toISOString().split('T')[0];

  const [form, setForm] = useState({
    InvoiceDate: today, InvoiceQuine: '', InvoiceNumber: '',
    InvoiceAmount: '', exp_paratp: '0', CostType: '', Stext: '',
    ppaytype: '1', BusArea: '', InvoiceSharedNumb: docnumb,
  });
  ...
  useEffect(() => { setForm(f => ({ ...f, InvoiceSharedNumb: docnumb })); }, [docnumb]);
    ...
    <>
      {message ? (
        ...
        <div style={{ maxWidth: 600, margin: '0 auto', padding: '24px 16px' }}>
          <h1 style={{ fontSize: 18, fontWeight: 700, color: 'var(--text)', marginBottom: 20 }}>Masraf Girişi</h1>
          <form onSubmit={handleSubmit} encType="multipart/form-data">
            <input type="hidden" name="ppaytype" value={showSharedRows ? form.ppaytype : '1'} />
            ...
            <div className="df-form-grid" style={{ gridTemplateColumns: '1fr' }}>
              {showSharedRows && (
                <>
                  <div className="df-field">
                    ...
                    <input className="df-inp" type="text" value={form.InvoiceSharedNumb} readOnly />
                  </div>
                  <div className="df-field">
                  ...
                  <div className="df-field">
                    <label className="df-field-lbl">Ödeme Tipi</label>
              ...
              <div className="df-field">
                <label className="df-field-lbl">Para Birimi</label>
            ...
            <table className="df-data-table">
              ...
              <tbody>
                {costTypes.length > 0 ? costTypes.map((ct, i) => (
                  <tr key={i} onClick={() => { setForm(f => ({ ...f, CostType: ct.MASRAFTIP })); setCostTypeName(ct.STEXT); setShowCostModal(false); }}>
            ...
            <table className="df-data-table">
              ...
              <tbody>
                {busAreas.length > 0 ? busAreas.map((ba, i) => (
                  <tr key={i} onClick={() => { setForm(f => ({ ...f, BusArea: ba.BUSAREA })); setBusAreaName(ba.STEXT); setShowBusModal(false); }}>
