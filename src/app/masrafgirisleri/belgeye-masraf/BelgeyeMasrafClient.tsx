...
interface SharedDoc {
  CLIENT: string; SHAREDNUMB: string; DOCDATE: string; STATUS: string; COMPANY: string;
}
interface InvDetail {
  CLIENT: string; EXTINVTYPE: string; EXTINVNUM: string;
  GROSSAMOUNT: string; DOCDATE: string; LTEXT: string; COSTX: string; COMPANY: string;
}
...
// HTML date input (YYYY-MM-DD) → CANIAS (YYYY/MM/DD)
function toCanias(iso: string) {
  return iso ? iso.replace(/-/g, '/') : '';
}

export default function BelgeyeMasrafClient() {
  const router = useRouter();
  const [filters, setFilters] = useState({ exp_comp: '', exp_date: todayISO(), exp_docn: '', exp_stat: '0', exp_islemtp: '0' });
  ...
  async function search() {
    setLoading(true);
    const payload = { ...filters, exp_date: toCanias(filters.exp_date) };
  ...
  async function newDoc() {
    const res  = await fetch('/api/expenses/new-doc', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' });
    const data = await res.json();
    if (data.status === 'OK') router.push(`/masrafgirisleri/masraf-girisi?docnumb=${data.response.replace('OK;', '')}`);
    else alert(data.response?.replace('FL;', '') || 'Hata oluştu');
  }

  async function addToDoc(doc: SharedDoc) {
    if (!doc.SHAREDNUMB) { alert('Belge No bulunamadı.'); return; }
    if (doc.STATUS === '2') { alert('Onaylı belgeye fiş eklenemez.'); return; }
    router.push(`/masrafgirisleri/masraf-girisi?docnumb=${doc.SHAREDNUMB}`);
  }
  ...
  async function loadDetail(doc: SharedDoc) {
    ...
    const res  = await fetch('/api/expenses/list-sh', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ comp: filters.exp_comp || '%', sh: doc.SHAREDNUMB }) });
    const data = await res.json();
    try {
      const parsed = typeof data.data === 'string' ? JSON.parse(data.data) : data.data;
      const rows = Array.isArray(parsed) ? parsed : parsed?.Records?.ROW;
  ...
  async function confirmDoc(doc: SharedDoc) {
    await fetch('/api/expenses/confirm-sh', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ comp: doc.COMPANY, sharenumb: doc.SHAREDNUMB }) });
    search();
  }

  async function showFis(d: InvDetail) {
    setFisModal({ open: true, loading: true });
    const res  = await fetch('/api/expenses/picture', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ comp: d.COMPANY || '', typp: d.EXTINVTYPE, numm: d.EXTINVNUM }) });
    const data = await res.json();
    try {
      const parsed = typeof data.data === 'string' ? JSON.parse(data.data) : data.data;
      const imgSrc = buildImgSrc(extractPictureRow(parsed));
    ...
    <>
      <div className="df-page">
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
          ...
          <button className="df-btn-yeni" onClick={newDoc}>Yeni</button>
        </div>
        ...
        <div className="df-table-wrap">
          <table className="df-data-table">
            <thead>
              <tr><th>Mağaza</th><th>Belge No</th><th>Tarih</th><th>Ekle</th><th>Onay</th><th>Red</th></tr>
            </thead>
            <tbody>
              {docs.map((doc, i) => (
                <Fragment key={i}>
                  <tr>
                    ...
                    <td>{doc.DOCDATE}</td>
                    <td><button className="df-btn-ekle" onClick={() => addToDoc(doc)}>Ekle</button></td>
                    <td>
                    ...
                    <td>
                      {doc.STATUS === '1' ? <span className="df-badge-red">Red</span>
                        : doc.STATUS === '0' ? <button className="df-btn-red" onClick={() => setRefuseModal({ open: true, doc })}>Red</button>
                        : null}
                    </td>
                  </tr>
                    ...
                    <tr>
                      <td colSpan={6} style={{ padding: 0 }}>
                        <table className="df-data-table df-child-table">
                          <thead><tr><th>Mağaza</th><th>Fat.Seri</th><th>Fat.No</th><th>Tutar</th><th>Tarih</th><th>Açıklama</th><th>Gider Açk.</th><th>Fiş</th></tr></thead>
                          <tbody>
                            {(details[doc.SHAREDNUMB] || []).map((d, j) => (
                              <tr key={j}><td>{d.CLIENT}</td><td>{d.EXTINVTYPE}</td><td>{d.EXTINVNUM}</td><td>{d.GROSSAMOUNT}</td><td>{d.DOCDATE}</td><td>{d.LTEXT}</td><td>{d.COSTX}</td><td><button className="df-btn-fis" onClick={() => showFis(d)}>Fiş</button></td></tr>
                            ))}
              ...
              {!docs.length && !loading && <tr><td colSpan={6} style={{ textAlign: 'center', padding: 20 }}>Arama yapın</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
