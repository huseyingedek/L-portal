...
export default function BayiOdemeleriPage() {
  ...
  async function handleKayit(e: React.FormEvent) {
    e.preventDefault();
    setLoading(true);
    const res = await fetch('/api/bayi-odemeleri/register', {
      method: 'POST',
  ...
  async function handleGiris(e: React.FormEvent) {
    e.preventDefault();
    setLoading(true);
    const res = await fetch('/api/bayi-odemeleri/login', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    setLoading(false);
    if (data.success) {
      router.push('/bayi-odemeleri/panel');
    } else {
