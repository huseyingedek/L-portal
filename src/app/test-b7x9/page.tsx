...
export default function BarkodTestPage() {
      ...
      try {
        ...
        if (!res.ok) {
          setSonuclar(prev => prev.map(s =>
            s.barkod === barkod ? { ...s, durum: 'hata', hata: `HTTP ${res.status}` } : s
          ));
          continue;
        }
        ...
        if (data.error) {
          setSonuclar(prev => prev.map(s =>
            s.barkod === barkod ? { ...s, durum: 'hata', hata: data.error } : s
          ));
          continue;
        }

        // İlk satırı al (ROW veya numerik key)
        ...
        setSonuclar(prev => prev.map(s =>
          s.barkod === barkod ? { ...s, durum: 'hata', hata: 'Bağlantı hatası' } : s
        ));
      }
    }
  ...
  const hatalı   = sonuclar.filter(s => s.durum === 'hata').length;
