...
if (empty($_POST['kisi_adres'])) {
	$kisi_adres = $kisi_ilce . "/" . strtoupper($kisi_sehir) . "/ ". $kisi_ulke;
} else {
...
if(isset($_POST['kisi_not'])){
	$kisi_not = $_POST['kisi_not'];
}else{
	$kisi_not = '';
}
...
$_SESSION['kvkk'] = $kvkk_code;
$_SESSION['etk'] = $etk_code;
$_SESSION['kvkk_etk_codes'] = $kvkk_etk_codes;
...
if (!empty($sonuc)) {
	$kisiVerileri = array(
		...
		'kisi_vergi_dairesi' => $kisi_vergi_dairesi,
		'kisi_dogum_tarihi' => $kisi_dogum_tarihi,
		'kisi_es_dogum_tarihi' => $kisi_es_dogum_tarihi,
		'kisi_evlilik_tarihi' => $kisi_evlilik_tarihi,
		'kisi_yuzuk_olcusu' => $kisi_yuzuk_olcusu,
		'kisi_pasaport_numarasi' => $kisi_pasaport_numarasi,
		'kisi_pasaport_tarihi' => $kisi_pasaport_tarihi,
		'kisi_ulke' => $kisi_ulke,
		'kisi_sehir' => $kisi_sehir,
		'kisi_ilce' => $kisi_ilce,
		'kisi_adres' => $kisi_adres,
		'kisi_not' => $kisi_not
	);

	$_SESSION['kisiVerileri'] = $kisiVerileri;
?>
	<!DOCTYPE html>
	<html>
	...
	<body>
		<script>
			function openPopup() {
				...
				xhr.open("POST", "sms_verification.php", true);
				...
				xhr.onreadystatechange = function() {
					if (xhr.readyState === 4 && xhr.status === 200) {
						...
						} else if (response === "ETK") {
							alert("Yalnızca ETK Kabul Edildi!");
							window.location.href = "https://online.lizaypirlanta.com/magazacilik/musteri-kaydi/";
						} else if (response === "KVKK_ETK") {
