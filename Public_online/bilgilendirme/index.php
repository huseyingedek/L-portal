...
$query = "SELECT * FROM `BILGILENDIRMELER` ORDER BY `BILGILENDIRMELER`.`bilgilendirme_tarih` DESC";
...
<style>
  .card-text {
    white-space: nowrap; /* Başlıkları satır sonunda kes */
      ...
      <div class="container">
        ...
        <?php if ($result) {
            ...
            foreach ($result as $row) {
              ...
              $bilgilendirme_dosya_adi = $row['bilgilendirme_dosya_adi'];
              ...
              if (!file_exists($dosya_adi)) {
                ...
                \$query = \"SELECT * FROM `BILGILENDIRMELER` WHERE bilgilendirme_dosya_adi = '$dosya_adi'\";
                try {
                    \$result = \$db->select(\$query);
                } catch (PDOException \$e) {
                    echo \"Sorgu hatası: \" . \$e->getMessage();
    ...
    <a href="<?php echo $dosya_adi; ?>" style="text-decoration:none;">
        <div class="card mb-4 shadow-sm flip-2-hor-bottom-1">
            <svg class="bd-placeholder-img card-img-top" width="100%" height="225" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" focusable="false" role="img" aria-label="Placeholder: Thumbnail">
                <title><?php echo $bilgilendirme_baslik; ?></title>
                <rect width="100%" height="100%" fill="<?php echo $rect_rengi; ?>" />
                <text x="50%" y="50%" fill="#eceeef" dy=".3em">
                  <tspan font-weight="bold"><?php echo $bilgilendirme_gorsel_baslik; ?></tspan></text>
            </svg>
            <div class="card-body">
                <p class="card-text" style="color: black;"><?php echo $bilgilendirme_baslik; ?></p>
            </div>
        </div>
    </a>
</div>
